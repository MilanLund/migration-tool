...
To be able to import data successfully to Kentico Cloud you need to follow the predefined JSON or CSV structure and fit your data in the structure.
...
8. Imports data in the Kentico Cloud project. In case of language variants, taxonomy groups, sitemap locations or other imported information do not exist in the Kentico Cloud project, the import process will fail and content items imported during the process get deleted. 
...
If the process fails the tool sends more requests to delete already imported content items. One request for each content item. There is no need to send additional delete requests for language variants. This is done to preserve the state of the Kentico Cloud project before the import process has had started. This is important to know because this could make you exceed the number of Content API calls in your Kentico Cloud pricing variant. I recommend you to test the import process on a small number of content items.
